...
const SECUREPAY_API_URL = 'https://api.securepay.com.br';
...
async function getAccessToken() {
    ...
    try {
        const response = await axios.post(`${SECUREPAY_API_URL}/api/partner/v1/auth-token`, {
            client_id: SECUREPAY_PUBLIC_KEY,
            client_secret: SECUREPAY_SECRET_KEY
        });
...
app.post('/create-payment', async (req, res) => {
    try {
        ...
        const response = await axios.post(`${SECUREPAY_API_URL}/api/partner/v1/cash-in`, paymentData, {
            headers: {
